...
//          dest_address = sharedPref.getString("dest","");
            ...
            reference = FirebaseDatabase.getInstance().getReference().child("Driver_Route");
    ...
    private void search_customers() {
        try {
            ...
            reference1 = FirebaseDatabase.getInstance().getReference().child("Cust_Route");
...
//
//        reference2 = FirebaseDatabase.getInstance().getReference().child("Cust_Route").child(key);
//        // TODO: handle loggedInUser authentication
...
//                if (snapshot.exists()) {
//                    for(DataSnapshot ds : snapshot.getChildren()) {
//                        latitude1[i]=ds.child("latitude").getValue(double.class);
//                        longitude1[i]=ds.child("longitude").getValue(double.class);
//                        i++;
//                    }
//                    matchpoints(latitude1,longitude1);
//                }
//
//            }
//
    ...
    public void matchpoints(LatLng l1,LatLng l2) {
        try {
            ...
            if (s1lat == s2lat && s1lng == s2lng)
            {
                ...
                if (s1lat == s2lat && s1lng == s2lng)
                {
                    place1 = new MarkerOptions().position(new LatLng(l1.latitude, l1.longitude)).icon(BitmapDescriptorFactory.fromResource(R.drawable.person_marker1)).title(start_address);
